...
int main(int argc, char* argv[])
{
  if (argc == 2 and string(argv[1]) == string("test")) {
    ...
    // FIXED
    size_t unique_mod_count = countFlags(flags, 42);
    cout << unique_mod_count << endl;
...
void readData(unsigned int * data, size_t arr_len) {
  ...
  //this code doesnt work you need to change int to size_t
...
void testCountFlags() {
  ...
  // FIXED
  initFlags(flags, 42);
  flags[getModulo42(13)] = true;
  flags[getModulo42(55)] = true;
  flags[getModulo42(98)] = true;
  assert(countFlags(flags, 42) == 3);

  initFlags(flags, 42);
  flags[getModulo42(7)] = true;
  flags[getModulo42(49)] = true;
